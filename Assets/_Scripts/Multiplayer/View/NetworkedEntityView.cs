...
using UnityEngine;
    ...
    public class NetworkedEntityView : ColyseusNetworkedEntityView
    {
        ...
        {
            while (MultiPlayerGameManager.Instance.IsInRoom == false)
            {
                yield return 0;
            }
            ...
            {
            MultiPlayerGameManager.CreateNetworkedEntity(prefabName, null, this);
            }
        ...
        public void InitiView(NetworkedEntity entity)
        {
            ...
            {
                state = entity;
                IsMine = MultiPlayerGameManager.Instance.CurrentUser != null && string.Equals(MultiPlayerGameManager.Instance.CurrentUser.sessionId, state.ownerId);
        ...
        protected virtual void UpdateStateFromView()
        {
            ...
            MultiPlayerGameManager.NetSend("entityUpdate", changeSet);
            }
        ...
        protected virtual void ProcessViewSync()
        {
            ...
            double interpolationTime = MultiPlayerGameManager.Instance.GetServerTime - interpolationBackTimeMs;
            ...
            if (proxyStates[0].timestamp > interpolationTime)
            {
                ...
                float deltaFactor = (MultiPlayerGameManager.Instance.GetServerTimeSeconds > proxyStates[0].timestamp) ?
                    (float)(MultiPlayerGameManager.Instance.GetServerTimeSeconds - proxyStates[0].timestamp) * 0.2f : 0f;
        ...
        {
        MultiPlayerGameManager.NetSend("setAttribute", new ExampleAttributeUpdateMessage() { entityId = state.id, attributesToSet = attributesToSet });
        }
