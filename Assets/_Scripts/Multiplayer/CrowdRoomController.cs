...
/// </summary>
[Serializable]
public class CrowdRoomController
{
    // Network Events
    //==========================
    /// <summary>
    ...
    /// </summary>
    [SerializeField]
    private static NetworkedUser _currentNetworkedUser;
    ...
    private void OnUserAdd(string key, NetworkedUser user)
    {
        ...
        user.OnChange += changes =>
        {
            ...
            if (MultiPlayerGameManager.Instance.CurrentUser != null &&
                string.Equals(MultiPlayerGameManager.Instance.CurrentUser.sessionId, user.sessionId))
            {
