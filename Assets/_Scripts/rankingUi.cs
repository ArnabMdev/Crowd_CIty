...
public class rankingUi : MonoBehaviour
{
    ...
    void Update()
    {
        if (GameManager.isGameStarted)
        {
            ...
            if (data.Count > 1)
            {
                firstText.text = "" + data[0].LeaderName + "  " + data[0].GroupCount;
                firstText.color = data[0].groupColor;
            }
            if (data.Count > 1)
            {
                secondText.text = "" + data[1].LeaderName + "  " + data[1].GroupCount;
                secondText.color = data[1].groupColor;
            }
            if (data.Count > 2)
            {

                thirdText.text = "" + data[2].LeaderName + "  " + data[2].GroupCount;
